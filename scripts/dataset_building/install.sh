source dataset_building/config.sh
...
reorganize_choy2016() {
  ...
  metadata_file="$choy_img_path/$modelname/rendering/camera_metadata.txt"
  ...
  python dataset_building/get_r2n2_cameras.py $metadata_file $camera_out_file
  counter=0
  for f in $img_dir/*.png; do
    outname="$(printf '%d.jpg' $counter)"
    echo $f
...
for c in ${CLASSES[@]}; do
  ...
  CHOY2016_IMG_PATH_C="$CHOY2016_PATH/$c"
  CHOY2016_VOX_PATH_C="$CHOY2016_PATH/$c"
  ...
  echo "Creating split"
  python create_split.py $OUTPUT_PATH_C --r_val 0.1 --r_test 0.1
done
